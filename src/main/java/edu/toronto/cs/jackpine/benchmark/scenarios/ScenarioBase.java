...
public abstract class ScenarioBase implements Scenario {
    ...
    public void initialize( Properties properties ) throws Exception {
        Column[] columns = new Column[]{ new Column( "mykey", Types.INTEGER, -1, -1, true, true ), new Column( "mydata", Types.INTEGER ), new Column( "mypayload", Types.VARCHAR, datawidth ) };
        tableSet = new TableSet( "benchmark_scenario_", tables, datarows, columns );
    ...
    public void globalPrepare() throws Exception {
        ...
        if ( reusedata ) {
            log.info( "Reusing existing test tables..." );
        } else {
            log.info( "Creating and populating test tables..." );
