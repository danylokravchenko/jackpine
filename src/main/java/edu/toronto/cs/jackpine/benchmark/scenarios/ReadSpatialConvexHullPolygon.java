 ...
 */
@Slf4j
public class ReadSpatialConvexHullPolygon extends SpatialScenarioBase {
    ...
    public void prepare() throws Exception {
        ...
        if ( dialect.getSqlDialectType() == SpatialSqlDialect.SupportedSqlDialect.Mysql ) {
            log.error( "ReadSpatialConvexHullPolygon not supported for Mysql" );
        } else {
            pstmtArray = new PreparedStatement[1];
            String sql = dialect.getSelectConvexHullPolygon();
