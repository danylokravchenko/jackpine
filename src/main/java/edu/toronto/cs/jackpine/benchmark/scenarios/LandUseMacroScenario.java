 ...
 */
@Slf4j
public class LandUseMacroScenario extends SpatialScenarioBase {
    ...
    public void prepare() throws Exception {
        ...
        String[] sql = dialect.getLandUseQueries();

        pstmtArray = new PreparedStatement[sql.length];

        for ( int i = 0; i < sql.length; i++ ) {
            log.warn( "????? preparing: " + sql[i] );
    ...
    public void iterate( long iterationCount ) throws Exception {
        for ( int i = 0; i < pstmtArray.length; i++ ) {
            System.out.println( "##### executing: " + pstmtArray[i].toString() );
            log.warn( "##### executing: " + pstmtArray[i].toString() );
